...
def main():
    ...
    translated_output_data_dir = os.path.join(
        data_dir, "tulu3_sft_instruction_following"
    )
    ...
    dataset = datasets.load_dataset(
        "allenai/tulu-3-sft-personas-instruction-following", split="train"
    )

    # renaming "messages" to "conversations"
    dataset = dataset.rename_column("messages", "conversations")

    # rename "content" to "value" and "role" to "from" in each message inside "conversations"
    def convert_content_to_value(example):
        example["conversations"] = [
            {
                **{k: v for k, v in msg.items() if k not in ("content", "role")},
                "value": msg.get("content", msg.get("value", "")),
                "from": msg.get("role", msg.get("from", "")),
            }
            for msg in example["conversations"]
        ]
        return example

    dataset = dataset.map(convert_content_to_value, num_proc=mp.cpu_count())
    ...
    if all_results:
        ...
        output_path = os.path.join(
            translated_output_data_dir, "tulu3_sft_instruction_following.parquet"
        )
