...
If you encounter code, return the code as is, but translate code comments.
...
class AsyncTranslator:
    ...
    async def translate(self, text: str, max_retries: int = 40) -> str:
        """Translate a single text from English to Ukrainian with async retry logic."""
        text = text.strip()

        if len(text) <= 5000:
            messages = [
                {"role": "user", "content": self.system_prompt.format(document=text)},
            ]
...
async def process_batch_async(
    ...
    async def process_single_example(example):
        async with semaphore:  # Limit concurrent requests
            try:
                original_instruction = example["messages"][0]["content"]
                instruction = await translator.translate(original_instruction)
...
def main():
    ...
    async def test_translation():
        try:
            result = await test_translator.translate("Hello, how are you?")
    ...
    dataset = datasets.load_dataset("chimbiwide/RolePlay-NPC", split="train")
    ...
    if all_results:
        ...
        output_path = os.path.join(
            translated_output_data_dir, "roleplay_npc_instruction.parquet"
        )
