...
def main():
    ...
    test_translator = AsyncTranslator(**config)
    ...
    dataset_name = "nvidia/Llama-Nemotron-VLM-Dataset-v1"

    # Setup directories
    cur_dir = os.getcwd()
    data_dir = f"{cur_dir}/data/"
    translated_output_data_dir = os.path.join(data_dir, dataset_name.replace("/", "_"))
    ...
    print(
        f"Dataset size before filtering: {sum(len(dataset[split]) for split in dataset.keys())}"
    )

    dataset = dataset.map(
        lambda x: {"too_long": prefilter_long_chunks(x)}, num_proc=min(32, num_cores)
    )
    dataset = dataset.filter(lambda x: x["too_long"])
    dataset = dataset.remove_columns("too_long")
    print(
        f"Dataset size after filtering: {sum(len(dataset[split]) for split in dataset.keys())}"
    )
    ...
    for split in dataset.keys():
        ...
        print(f"Dataset split '{split}' has {len(dataset[split])} examples")

        if args.test_mode:
            dataset[split] = dataset[split].select(range(100))  # Limit for testing
            print("Running in test mode with 100 examples")

        print(f"Dataset loaded with {len(dataset[split])} examples")

        # Split dataset for workers
        print(f"Splitting dataset for {args.num_workers} workers...")
        data_chunks = split_dataset(dataset[split], args.num_workers)
        ...
        progress_proc = Process(
            target=progress_monitor, args=(progress_queue, len(dataset[split]))
        )
        progress_proc.start()
        ...
        if all_results:
            ...
            output_path = os.path.join(
                translated_output_data_dir,
                f"{dataset_name.replace('/', '_')}_{split}.parquet",
            )
